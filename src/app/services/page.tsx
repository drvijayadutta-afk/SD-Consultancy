...
const services = [
  {
    ...
    overview: 'Strategic mass media campaigns that build brand awareness, establish thought leadership, and create lasting brand equity for healthcare organizations across all major traditional channels.',
    benefits: ['Broad reach for new patient acquisition', 'Brand recall and top-of-mind positioning', 'Physician and stakeholder trust-building', 'Integrated multi-channel impact'],
    offerings: [
  ...
  },
  {
    ...
    overview: 'Targeted, relationship-driven communications that build deep connections with physicians, patients, and community stakeholders through meaningful healthcare activations and engagement programs.',
    benefits: ['Direct physician relationship building', 'Targeted patient community engagement', 'Measurable referral network growth', 'High-conversion touchpoint design'],
    offerings: [
  ...
  },
  {
    ...
    overview: 'Comprehensive digital ecosystem design that connects healthcare organizations with patients, physicians, and stakeholders across every digital touchpoint in their decision journey.',
    benefits: ['Accelerated online patient acquisition', 'SEO-driven organic visibility', 'Integrated digital patient journey', 'Data-driven optimization'],
    offerings: [
  ...
  },
  {
    ...
    overview: 'Purposeful creative excellence — brand films, patient stories, visual identity systems, and campaign concepts that build trust, inspire action, and elevate your healthcare brand.',
    benefits: ['Brand differentiation through storytelling', 'Emotional connection with patients', 'Clinical credibility through creative', 'Consistent visual identity system'],
    offerings: [
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedSection className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-5xl font-bold font-heading text-white leading-tight mb-6">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#4F8A77] to-[#B8924A]">Healthcare Market Leaders</span>
            </h1>
            <p className="text-lg text-white/65 leading-relaxed">From strategy to execution, we provide the full spectrum of brand, communications, and creative services that healthcare organizations need to achieve and sustain market leadership.</p>
          </AnimatedSection>
          <AnimatedSection delay={200} className="mt-10 flex flex-wrap gap-3">
            {services.map((s) => (
              <a key={s.id} href={`#${s.id}`} className="flex items-center gap-2 px-4 py-2 bg-white/10 border border-white/20 rounded-full text-sm text-white/80 hover:bg-white/20 transition-colors">
      ...
      {services.map((service, idx) => (
        <section key={service.id} id={service.id} className={`py-24 ${idx % 2 === 0 ? 'bg-white dark:bg-[#0f1923]' : 'bg-[#f8fafc] dark:bg-[#0d1520]'}`}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-16 items-start">
              <AnimatedSection>
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-6 text-sm font-semibold" style={{ color: service.color, backgroundColor: service.color + '15' }}>
                  <service.icon className="w-4 h-4" />
                  {service.label}
                </div>
                <h2 className="text-3xl md:text-4xl font-bold font-heading text-[#1F3A5F] dark:text-white mb-5">{service.title}</h2>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {service.deliverables.map((d) => (
                      <span key={d} className="text-xs px-3 py-1.5 rounded-full font-medium" style={{ color: service.color, backgroundColor: service.color + '12' }}>{d}</span>
                    ))}
                  </div>
                </div>
                <Link href="/contact" className="inline-flex items-center gap-2 mt-8 px-6 py-3 rounded-xl font-semibold text-white hover:-translate-y-0.5 transition-all" style={{ backgroundColor: service.color }}>
              ...
              <AnimatedSection direction="right" delay={200}>
                <div className="grid gap-4">
                  {service.offerings.map((offering) => (
                    <div key={offering.name} className="bg-white dark:bg-[#162d4a] rounded-xl p-5 border border-gray-100 dark:border-white/5 card-hover">
