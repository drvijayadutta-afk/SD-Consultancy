...
const testimonials = [
  ...
  { quote: "Working with SD Square Consulting on our MedTech go-to-market strategy was a game changer. They understood both the clinical and commercial dimensions in a way no other agency has.", name: 'Rajesh Nair', title: 'VP Marketing, MedTech Startup', sector: 'MedTech', initials: 'RN', avatarBg: SOLID_FILL.gold, textColor: ON_DARK.gold },
];

export function TestimonialsSection() {
  const [active, setActive] = useState(0);
  return (
    <section className="py-24 bg-gradient-to-br from-[#101E36] to-[#0f1923] relative overflow-hidden">
      <div className="absolute inset-0">
        <div className="absolute top-0 right-0 w-96 h-96 bg-[#25405F]/10 rounded-full blur-[120px]" />
        <div className="absolute bottom-0 left-0 w-80 h-80 bg-[#3E5776]/10 rounded-full blur-[100px]" />
      </div>
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <AnimatedSection>
          <div className="bg-white/5 backdrop-blur-md rounded-3xl border border-white/10 p-10 md:p-14">
            <Quote className="w-10 h-10 text-[#E8E0CB] mb-6 opacity-60" />
          ...
          <div className="flex justify-center gap-2 mt-8">
            {testimonials.map((_, i) => (<button key={i} onClick={() => setActive(i)} className={`rounded-full transition-all ${i === active ? 'w-8 h-2 bg-[#E8E0CB]' : 'w-2 h-2 bg-white/30'}`} />))}
          </div>
        </AnimatedSection>
      </div>
    </section>
