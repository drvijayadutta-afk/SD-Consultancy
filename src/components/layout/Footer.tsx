...
function FooterLogo() {
  return (
    <svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="S.D Consultancy Logo">
      ...
      <text x="22" y="20" textAnchor="middle" dominantBaseline="middle" fill="white" fontFamily="Space Grotesk, sans-serif" fontWeight="700" fontSize="13" letterSpacing="1">S·D</text>
      <path
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-[#1F3A5F] via-[#1a3358] to-[#162d4a]">
        <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-8">
          <svg viewBox="0 0 1200 80" preserveAspectRatio="none" className="absolute bottom-0 w-full h-20">
            <path d="M 0 50 L 120 50 L 150 30 L 180 70 L 210 40 L 240 50 L 400 50 C 500 50 600 30 700 20 C 800 10 900 15 1000 12 L 1200 8" stroke="white" strokeWidth="1.5" fill="none" strokeOpacity="0.15" />
          </svg>
        </div>
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              ...
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link.href}>
                    <Link href={link.href} className="text-sm text-white/65 hover:text-[#4F8A77] transition-colors flex items-center gap-1.5 group/link">
            ...
            {subscribed ? (
              ...
              <form onSubmit={handleSubscribe} className="flex gap-2 w-full md:w-auto">
                <input
                  ...
                  className="flex-1 md:w-64 px-4 py-2.5 bg-white/8 border border-white/15 rounded-lg text-sm text-white placeholder:text-white/40 focus:outline-none focus:border-[#2E75B6] transition-colors"
                />
                <button type="submit" className="px-5 py-2.5 bg-gradient-to-r from-[#2E75B6] to-[#4F8A77] text-white text-sm font-semibold rounded-lg hover:shadow-lg hover:shadow-[#2E75B6]/30 transition-all flex items-center gap-2">
                  Subscribe
                </button>
              </form>
            )}
        ...
        <div className="mt-10 pt-6 border-t border-white/8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6">
            {['Privacy Policy', 'Terms of Service', 'Cookie Policy'].map((item) => (
              <Link key={item} href="#" className="text-xs text-white/55 hover:text-white/75 transition-colors">
                {item}
              </Link>
            ))}
          </div>
