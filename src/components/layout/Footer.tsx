...
export function Footer() {
  ...
  return (
    <footer className="bg-[#0b1520] text-white">
      {/* CTA Banner */}
      <div className="relative overflow-hidden bg-gradient-to-br from-[#101E36] via-[#16283F] to-[#162d4a]">
        {/* Decorative ECG line */}
        ...
        <div className="absolute top-0 right-0 w-96 h-96 bg-[#25405F]/20 rounded-full blur-[100px] pointer-events-none" />
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-[#3E5776]/15 rounded-full blur-[80px] pointer-events-none" />

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          <div className="inline-flex items-center gap-2 mb-5 px-4 py-1.5 rounded-full border border-[#3E5776]/30 bg-[#3E5776]/10">
            <span className="w-1.5 h-1.5 rounded-full bg-[#3E5776] animate-pulse" />
            <span className="text-[#8393a7] text-xs font-semibold tracking-widest uppercase">Strategy Consultation</span>
          </div>
          ...
          <Link
            href="/contact"
            className="inline-flex items-center gap-2 px-8 py-4 bg-white text-[#101E36] font-bold rounded-xl hover:bg-[#E7EAEF] transition-all hover:-translate-y-1 shadow-xl hover:shadow-2xl hover:shadow-[#101E36]/30 group/cta"
          >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-3 mb-6 group">
              <FooterLogo />
              <div>
                <div className="text-white font-bold text-lg font-heading">SD Square</div>
                <div className="text-[#8393a7] text-[10px] font-semibold tracking-[0.18em] uppercase">Consulting</div>
              </div>
            </div>
            ...
            <p className="text-[#E8E0CB] text-xs font-medium italic mb-8">
              &ldquo;Clinical insight becoming market momentum.&rdquo;
            </p>

            {/* Contact */}
            <div className="space-y-3 mb-8">
              <a href="mailto:drvijayadutta@gmail.com" className="flex items-center gap-3 text-sm text-white/70 hover:text-[#8393a7] transition-colors group/contact">
                <span className="w-8 h-8 rounded-lg bg-white/8 flex items-center justify-center group-hover/contact:bg-[#3E5776]/20 transition-colors">
                  <Mail className="w-3.5 h-3.5 text-[#8393a7]" />
                </span>
              ...
              <div className="flex items-start gap-3 text-sm text-white/70">
                <span className="w-8 h-8 rounded-lg bg-white/8 flex items-center justify-center flex-shrink-0">
                  <MapPin className="w-3.5 h-3.5 text-[#8393a7]" />
                </span>
                New Delhi, India
              </div>
            </div>
            ...
            <div className="flex gap-2">
              {socials.map(({ symbol, href, label }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="w-9 h-9 rounded-lg bg-white/8 border border-white/10 flex items-center justify-center text-white/65 hover:bg-gradient-to-br hover:from-[#25405F] hover:to-[#3E5776] hover:text-white hover:border-transparent transition-all duration-200 text-xs font-bold"
                >
                  {symbol}
                </a>
              ))}
              ...
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-white/65 hover:text-[#8393a7] transition-colors flex items-center gap-1.5 group/link"
                    >
                      <ArrowRight className="w-3 h-3 opacity-0 -translate-x-1 group-hover/link:opacity-100 group-hover/link:translate-x-0 transition-all duration-200 text-[#8393a7]" />
                      {link.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-12 pt-10 border-t border-white/8">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            {subscribed ? (
              <div className="flex items-center gap-2 text-[#8393a7] font-medium">
              ...
              <form onSubmit={handleSubscribe} className="flex gap-2 w-full md:w-auto">
                <input
                  ...
                  required
                  className="flex-1 md:w-64 px-4 py-2.5 bg-white/8 border border-white/15 rounded-lg text-sm text-white placeholder:text-white/55 focus:outline-none focus:border-[#25405F] transition-colors"
                />
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-gradient-to-r from-[#25405F] to-[#3E5776] text-white text-sm font-semibold rounded-lg hover:shadow-lg hover:shadow-[#25405F]/30 transition-all flex items-center gap-2"
                >
                  Subscribe
                </button>
              </form>
        ...
        <div className="mt-10 pt-6 border-t border-white/8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-white/55">
            © {new Date().getFullYear()} SD Square Consulting. All rights reserved. Founded 2026.
          </p>
